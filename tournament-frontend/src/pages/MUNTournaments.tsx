...
const MUNTournaments = () => {
  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      try {
        ...
        // Filter for MUN tournaments only
        const munTournaments = data.filter(t => t.type === 'MUN');
        setTournaments(munTournaments);
        setLoading(false);
      } catch (err) {
        setError('Failed to fetch MUN tournaments.');
        setLoading(false);
        console.error(err);
      }
    };
  ...
  if (loading) {
    return (
      <Container>
        <Typography>Loading MUN tournaments...</Typography>
      </Container>
    );
  }
    ...
    <Container>
      <Box sx={{ py: 4 }}>
        <Typography variant="h4" component="h1" gutterBottom>
          Model United Nations (MUN) Tournaments
        </Typography>
          ...
          {filteredTournaments.length === 0 ? (
            <Grid item xs={12}>
              <Typography variant="h6" color="text.secondary" align="center">
                No MUN tournaments found matching your search criteria.
              </Typography>
            </Grid>
          ) : (
